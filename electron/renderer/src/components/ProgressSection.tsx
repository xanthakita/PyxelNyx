...
export default function ProgressSection({ fileProgress, overallProgress, currentFileName, statusMessage, statusColor }: Props) {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Typography variant="subtitle2" gutterBottom>{t.processingProgress}</Typography>
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
        <Box>
          <Typography variant="caption" color="text.secondary" fontWeight="bold">{t.individualFileProgress}</Typography>
        ...
        <Box>
          <Typography variant="caption" color="text.secondary" fontWeight="bold">{t.overallProgress}</Typography>
